...
from data_adapter.xml_adapter.exceptions import (
    XmlInvalidValueError,
...
class XmlExtractor:
    ...
    def find_node(
        self, path: str, parent: Optional[ET.Element] = None
    ) -> Optional[ET.Element]:
    ...
    def require_node(
        ...
        if node is None:
            raise XmlNodeNotFoundError(
                f"Node not found: path:{path}\nparent:{str(parent)}"
            )
        return node
    ...
    @overload
    def get_attr(
        self, node: Optional[ET.Element], attr: str, default: T
    ) -> Union[str, T]: ...

    def get_attr(
        self, node: Optional[ET.Element], attr: str, default: Optional[T] = None
