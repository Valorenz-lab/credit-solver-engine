...
from data_adapter.xml_adapter.serializers.serializer_bank_account import (
    serialize_bank_account,
)
from data_adapter.xml_adapter.serializers.serializer_checking_account import (
    serialize_checking_account,
)
from data_adapter.xml_adapter.serializers.serializer_credit_card import (
    serialize_credit_card,
)
from data_adapter.xml_adapter.serializers.serializer_global_debt import (
...
from data_adapter.xml_adapter.serializers.serializer_score_alert import (
...
def serialize_full_report(report: FullReport) -> SerializedFullReport:
    ...
    open_portfolio = [acc for acc in report.portfolio_accounts if acc.is_open]
    ...
    open_bank_accounts = [
        serialize_bank_account(acc) for acc in report.bank_accounts if acc.is_open
    ]
    closed_bank_accounts = [
        serialize_bank_account(acc) for acc in report.bank_accounts if not acc.is_open
    ]

    checking_accounts = [
        serialize_checking_account(acc) for acc in report.checking_accounts
    ]

    # Active obligations: open portfolio accounts + open credit cards
    active_portfolio = [
        _serialize_account(acc) for acc in report.portfolio_accounts if acc.is_open
    ]
    active_cards = [serialize_credit_card(c) for c in report.credit_cards if c.is_open]
    active_obligations: list[
        Union[SerializedPortfolioAccount, SerializedCreditCard]
    ] = [*active_portfolio, *active_cards]
    ...
    global_debt_records = [
        serialize_global_debt_record(g) for g in report.global_debt_records
    ]

    debt_evolution: list[SerializedDebtEvolutionQuarter] = (
        [
            serialize_debt_evolution_quarter(q)
            for q in report.aggregated_info.debt_evolution
        ]
        if report.aggregated_info is not None
        else []
    )
